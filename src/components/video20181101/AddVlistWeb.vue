      ...
      <div class="file-list">
        <ul class="file-item" :class="`file-${file.id}`" v-for="(file, index) in fileList" :key="index">
          ...
          <li class="file-operate">
            <!-- <a title="开始" @click="resume(file)"><i class="el-icon-upload2"></i></a>
            <a title="暂停" @click="stop(file)"><i class="el-icon-caret-right"></i></a> -->
            <a title="移除" @click="remove(file)"><i style="font-size:18px;" class="el-icon-delete"></i></a>
          </li>
